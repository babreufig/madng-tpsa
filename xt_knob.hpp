 ...
 *
 * Maps a strength double's buffer address to a parametric TPSA. The
 * generated wrappers (xt_knob_wrappers.h) call xt_knob() so that an element attribute
 * affected by a knob enters the physics as a TPSA in the knob parameters. Every other
 * field enters as a constant TPSA. The table is (re)set from Python before every
 ...
 * temporary type instead, which the getter wrappers hand back by value. */
...
/* Per-element strength-address slots (0..7 = k0,k1,k2,k3,k0s,k1s,k2s,k3s). A
 * magnet header affected by knobs fills the slots it uses (XT_KNOB_SET).
 * The dispatch clears them before every element (XT_KNOB_CLEAR)
 * so a non-setting element never reads stale addresses. */
